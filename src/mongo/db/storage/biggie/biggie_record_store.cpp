...
StatusWith<RecordId> RecordStore::insertRecord(OperationContext* opCtx,
    ...
    StringStore* workingCopy = getRecoveryUnitBranch_forking(opCtx);
    char* copyDat = (char*)std::malloc(len);
    std::memcpy(copyDat, data, len);
    workingCopy->insert(StringStore::value_type(key, std::string(copyDat, len)));
...
Status RecordStore::insertRecordsWithDocWriter(OperationContext* opCtx,
    ...
    for (size_t i = 0; i < nDocs; i++) {
        ...
        char* dat = (char*)std::malloc(len);
        docs[i]->writeDocument(dat);
        workingCopy->insert(StringStore::value_type(key, std::string(dat, len)));
...
bool RecordStore::Cursor::restore() {
    ...
    StringStore* workingCopy = getRecoveryUnitBranch_forking(opCtx);
    it = workingCopy->find(_savedPosition.value());
    return true;
}
...
bool RecordStore::ReverseCursor::restore() {
    ...
    StringStore* workingCopy = getRecoveryUnitBranch_forking(opCtx);
    it = StringStore::reverse_iterator(workingCopy->upper_bound(_savedPosition.value()));
    return true;
}
