...
public:
    ...
    size_type erase(const Key& key) {
        ...
        for (const uint8_t* it = static_cast<uint8_t>(key.data()); it != key.data() + key.size();
             ++it) {
    ...
    const_iterator lower_bound(const Key& key) const {
        ...
        for (const char* charKey = key.data(); charKey != key.data() + key.size(); ++charKey) {
            const uint8_t k = static_cast<const uint8_t>(*charKey);
            if (node->children[k] != nullptr) {
                // If the tree contains the node, keep following it.
                node = node->children[k].get();
            } else {
...
private:
    ...
    std::pair<const_iterator, bool> _upsertWithCopyOnSharedNodes(
        ...
        for (; i < key.size(); i++) {
            ...
            const uint8_t c = static_cast<uint8_t>(keyString[i]);
        ...
        std::cout << "~~~ going through key (string)" << key << std::endl;
        for (; i < key.size(); i++) {
            const uint8_t c = static_cast<uint8_t>(keyString[i]);
            std::cout << "~~~ 1 : " << i << " : " << c << " : " << (unsigned int)c << std::endl;
